---
name: v2-architecture-principles
description: Архитектурные принципы (SOLID, IoC/DI, слоёная архитектура), работа с диаграммами, инварианты конвейера v2. Применять при архитектурном проектировании, ревью архитектуры, анализе зависимостей, создании или разборе диаграмм, разметке ENGINEER_TASKS. Универсальный скилл — активен для любого TASK_DOMAIN.
---
...
- Зависимости между task_id описываются через `dependencies: [id]` — оркестратор запускает зависимые задачи только после завершения предшественников
- Если инженер обнаруживает невыполнимость — создаёт `*_blocker.md`, а не пишет код
- Задачи с `dependencies: []` запускаются параллельно — архитектор должен максимизировать их количество
...
- `dependencies: []` — задача независима, может запускаться параллельно с другими
- `dependencies: [id]` — задача ждёт завершения указанной
- `files_scope` — границы работы инженера; выход за них — повод для блокера
- Старайся минимизировать зависимости между task_id для максимального параллелизма
...
- [ ] Задачи в ENGINEER_TASKS максимально независимы (для параллельного dispatch)?
