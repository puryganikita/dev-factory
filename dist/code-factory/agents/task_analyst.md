...
3. Материализуй контекст из issue в локальные файлы:
   ...
   - **`design_context/`** — скачай файлы дизайн-контекста из comments issue:
     - Найди comments с заголовком `## Design Context:` — это файлы контекста компонентов. Сохрани каждый как отдельный `.md` файл в `design_context/`
     - Найди comments с заголовком `## Design Context Screenshot:` — это скриншоты из Figma. Извлеки URL изображения из markdown `![...](<url>)` и скачай:
       ```bash
       curl -sL "<raw_url>" -o design_context/<component-name>_screenshot.png
       ```
     - Если URL скриншота недоступен — сообщи пользователю, но продолжи работу
...
ai/tasks/task-<NN>-<краткое-описание>/
  ...
  design_context/           ← из comments issue
    ...
    <component>_screenshot.png  ← скриншоты из Figma (скачаны по ссылкам)
    ...
...
- design_context/<component>_screenshot.png — скриншот из Figma
```
