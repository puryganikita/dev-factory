...
ai/tasks/task-<NN>-<название>/subtasks/
  subtask-01-<name>/
    task_spec.md
    design_context/       ← скопируй релевантные файлы из ../design_context/ (включая *.png скриншоты)
  subtask-02-<name>/
    task_spec.md
    design_context/
  ...
...
- Копируй в `subtask-NN/design_context/` только те файлы из `design_context/`, которые релевантны ЭТОЙ подзадаче (включая `.png` скриншоты — они обязательная часть контекста)
