...
Если `requirements_lock.md` не найден — сообщи пользователю и остановись.
...
Перед финализацией архитектурного решения **обязательно**:

1. Прочитай секцию `API_SIGNATURES` в `requirements_lock.md`
2. Если твой дизайн типов/контрактов отклоняется от сигнатуры, указанной пользователем (`SIG-*`), ты **ОБЯЗАН** явно обосновать отклонение в секции «Обзор решения» в `architect_output.md`
3. Проверь что ни один `FORBIDDEN-*` не нарушается архитектурным решением
4. Проверь что все `REQ-*` покрыты архитектурой

## Feedback: вопросы к пользователю

Если при проектировании обнаружены неясности, требующие ввода пользователя:

1. Создай файл `ai/tasks/task-<NN>-<название>/architect_questions.md`
2. Установи `STATUS: NEEDS_CLARIFICATION` в начале `architect_output.md`
3. Формулируй вопросы на языке `USER_LANGUAGE` из `requirements_lock.md` в формате квиза (см. скилл `asking-questions-to-user`)
4. Оркестратор маршрутизирует вопросы к task_analyst → пользователю

## Правила
- Не пиши реализацию — только архитектуру и контракты
- **Проектирование типов, сигнатур и контрактов — это твоя зона ответственности** (не аналитика)
...
- **Сверка с `requirements_lock.md` обязательна** перед финализацией — см. секцию выше
