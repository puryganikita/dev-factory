...
0. **requirements_lock.md** — прочитай первым. Это неизменяемый источник требований пользователя. Используй как acceptance criteria при реализации
...
`requirements_lock.md` — неизменяемый источник требований пользователя. Он содержит:
- `REQ-*` — обязательные требования (дословные цитаты из задачи)
- `FORBIDDEN-*` — явные запреты
- `SIG-*` — API-сигнатуры, указанные пользователем

**Правило:** `architect_output.md` — твой основной вход для реализации. `requirements_lock.md` — твои acceptance criteria. Если реализация по `architect_output.md` расходится с `requirements_lock.md` — это повод для блокера с типом `requirements-mismatch`.
...
- [ ] Прочитан `requirements_lock.md`? Релевантные `REQ-*` учтены в реализации?
- [ ] Ни один `FORBIDDEN-*` из `requirements_lock.md` не нарушен?
- [ ] `SIG-*` из `API_SIGNATURES` реализованы точно (или отклонение обосновано архитектором)?
